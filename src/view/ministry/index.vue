<template>
  <div class="p-24px">
    ...
    <div class="flex items-center justify-between mb-24px">
      <div>
        <h1 class="text-24px font-600 text-gray-800 mb-8px">服侍者管理</h1>
        <p class="text-14px text-gray-500">管理营会服侍人员和服侍类型</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-16px mb-24px">
      <a-card size="small" class="text-center">
        <div class="text-24px font-600 text-blue-600 mb-4px">{{ members.length }}</div>
        <div class="text-12px text-gray-600">总服侍者</div>
      </a-card>
      
      <a-card size="small" class="text-center">
        <div class="text-24px font-600 text-green-600 mb-4px">{{ ministries.length }}</div>
        <div class="text-12px text-gray-600">服侍类型</div>
      </a-card>
      
      <a-card size="small" class="text-center">
        <div class="text-24px font-600 text-orange-600 mb-4px">{{ activeMembers }}</div>
        <div class="text-12px text-gray-600">活跃成员</div>
      </a-card>
    </div>
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <div class="text-12px text-gray-500">
                    <span class="mr-16px">
                      <i class="i-carbon:time mr-4px" />{{ member.availability }}
                    </span>
                    <span v-if="member.skills.length">
                      <i class="i-carbon:skill-level mr-4px" />{{ member.skills.join(', ') }}
                    </span>
                  </div>
                </div>
        ...
        <a-card title="服侍者详情" size="small">
          <div v-if="selectedMember">
            <div class="space-y-12px">
              ...
              <div>
                <div class="text-12px text-gray-500 mb-4px">服侍类型</div>
                <div class="space-y-4px">
                  <a-tag
                    v-for="ministryId in selectedMember.ministryIds"
                    :key="ministryId"
                    color="blue"
                  >
                    {{ getMinistryName(ministryId) }}
              ...
              <div>
                ...
                <div class="text-14px text-gray-800">{{ selectedMember.skills.join(', ') || '暂无' }}</div>
              </div>
    ...
    >
      ...
      >
        ...
        <a-form-item label="可服侍时间">
          <a-input v-model:value="formData.availability" placeholder="如：周末、工作日晚上等" />
        </a-form-item>
...
const activeMembers = computed(() => {
  return members.value.filter(member => 
    activities.value.some(activity => 
      activity.assignedMembers.includes(member.id)
    )
  ).length
})
