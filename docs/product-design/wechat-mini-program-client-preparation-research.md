...
6. 但是，当前产品范围明确：报告模块只把用户带到现有商城商品详情，购物车、订单和支付仍由现有商城负责，报告模块本身不处理订单或支付。[本项目业务设计](./pet-health-report-business-design.md) 因此不能把“新申请商户号”写成当前报告项目的无条件事实，必须先确认商城与报告使用的是同一小程序 AppID 还是独立小程序。
...
- **微信支付**：只在当前 AppID 直接调起支付，或现有商城本身就在当前 AppID 内承接支付时适用。若报告小程序仅跳转到另一个已运营的商城小程序，由商城 AppID 完成订单和支付，则报告小程序本身不需要新商户号或支付密钥。
...
- 如果同一小程序直接销售宠物商品，还可能涉及“商家自营—宠物食品/用品”；若只是跳转现有商城，则应以实际承接交易的商城小程序类目为准。
...
对当前项目，至少要确认微信身份标识与平台用户的关联方式、手机号是否处理、宠物与报告信息的保存期限、商城跳转中是否向商城传递用户或报告来源信息、用户请求删除或更正信息时由谁处理。未确定的接口不能先写入指引，实际调用的接口也不能漏报。
...
当前产品设计明确排除报告模块内的购物车、订单、支付和交易归因，报告中的可购商品只进入现有商城 SPU 详情。[本项目业务设计](./pet-health-report-business-design.md)

因此甲方必须先书面确认以下三种情况之一：

1. **同一 AppID 内已有商城和支付**：不新申请商户号，先核验现有商户号的 JSAPI/小程序支付权限、主体、费率、风险状态以及与当前 AppID 的绑定。
2. **报告小程序跳转到独立商城小程序**：支付能力属于商城 AppID；报告小程序只需完成跳转和商城可用性验证，不应向报告项目索要新的支付密钥。
3. **新增需求：报告小程序直接下单支付**：这超出当前范围，需要为当前 AppID 完整准备商户号、产品权限、绑定、安全参数、回调和真实交易测试，并重新评估开发任务和工期。
...
- **新申请商户号**：仅在没有可复用商户号且当前 AppID 要直接支付时办理。需要按主体类型提交营业执照或登记证书、法人/经营者证件和结算账户等材料，完成资料提交、账户验证、审核和签约。[微信支付：小程序接入申请指引](https://pay.wechatpay.cn/static/applyment_guide/applyment_detail_miniapp.shtml)、[腾讯客服：微信支付商户入驻支持的主体类型](https://kf.qq.com/faq/180910IBZVnQ180910naQ77b.html)
...
当前 AppID 直接支付时，甲方商户超级管理员需要给技术负责人配置商户平台账号和安全联系人，并安全交付：商户号、AppID、商户 API 证书、商户 API 私钥、证书序列号、APIv3 密钥，以及微信支付公钥和公钥 ID；既有系统如果仍使用平台证书，则平台证书与微信支付公钥是方案选择，不应写成两套都必须提供。[微信支付：开发必要参数说明](https://pay.wechatpay.cn/doc/v3/merchant/4013070756)、[微信支付：APIv3 密钥](https://pay.wechatpay.cn/doc/v3/merchant/4012072195)、[微信支付：商户 API 证书](https://pay.wechatpay.cn/doc/v3/merchant/4012072428)、[微信支付：微信支付公钥](https://pay.wechatpay.cn/doc/v3/merchant/4012153196)
...
| `pre-mp-certification` | 微信认证状态 | 条件项：本 AppID 支付或能力要求认证时 | 甲方业务负责人、财务 | 第 1 周 | 后台显示认证有效；若不需要认证，形成“不适用”结论及依据，不能长期显示为未完成 | `mp-01`, `mp-05`, `ops-06` |
...
| `pre-pay-path` | 商城支付承接方式确认 | 必需：确认结论；支付实施为条件项 | 甲方业务负责人、财务、技术负责人 | 第 1 周 | 书面确认“同 AppID 复用商城支付 / 跳转独立商城小程序 / 新增当前 AppID 直接支付”之一，并记录目标商城 AppID 和商户号；范围与产品设计一致 | `mp-04`, `mp-05`, `ops-04`, `ops-06` |
| `pre-pay-merchant` | 商户号与 JSAPI/小程序支付权限 | 条件项：本 AppID 直接支付 | 甲方财务、商户超级管理员 | 第 1 周启动 | 新申请：入驻、账户验证、审核、签约和权限开通完成；复用：现有商户号主体、费率、风险状态、JSAPI 权限核验通过，不重复申请 | `mp-04`, `mp-05`, `ops-04` |
| `pre-pay-binding-security` | 商户号绑定 AppID 与支付安全参数 | 条件项：本 AppID 直接支付 | 商户超级管理员、小程序管理员、技术负责人 | 第 6 周 | 当前 AppID 绑定已由双方后台确认；技术账号已配置；商户 API 私钥/证书、序列号、APIv3 密钥及微信支付公钥方案安全交付，未进入前端或公开仓库 | `mp-04`, `mp-05`, `ops-04`, `ops-05` |
| `pre-pay-callback-test` | 支付回调与真实支付退款闭环 | 条件项：本 AppID 直接支付 | 甲方财务、IT、测试负责人 | 第 8 周 | 公网 HTTPS 支付/退款通知可达；防火墙/WAF 不拦截；真实小额支付、查单、支付通知、退款、退款通知/查询全部通过并留存结果 | `mp-04`, `mp-05`, `ops-04`, `ops-05`, `ops-06` |

## 9. 页面呈现建议

- 顶部“未完成待配合”只统计当前确实适用且尚未完成的事项。
- 条件项应显示条件，例如“微信支付（仅本小程序直接支付时）”，在承接方式未确认前先突出 `pre-pay-path`，不要把新商户号、密钥、回调全部算成甲方已经欠缺。
...
2. **商城与报告的 AppID 关系**：产品设计只确认报告跳转现有商城，没有确认二者是同一 AppID、跨小程序跳转还是 H5 商城。该事实决定支付、业务域名和类目由谁承接。
3. **现有商城商户号状态**：本研究未登录甲方微信支付商户平台，无法确认商户号、主体、费率、风险状态、JSAPI 权限和 AppID 绑定现状。
4. **实际使用的域名类型**：是否需要 `downloadFile`、`uploadFile`、`socket` 或 `web-view` 取决于最终实现；官方规则只证明这些能力各自需要配置，不能据此断言当前项目全部需要。
5. **认证是否为当前 AppID 的发布硬条件**：官方资料明确证明支付权限要求小程序认证，但没有证明本项目所有报告阅读功能都必须先完成微信认证。因此页面应保留条件说明，而不是把认证扩大成所有开发的无条件阻塞项。
